...
type
  ...
  private
    ...
    procedure SetaCamposInteger(ATabela: TObject; ARecParams: TRecParams);
    procedure SetaCamposString(ATabela: TObject; ARecParams: TRecParams);
    procedure SetaCamposDate(ATabela: TObject; ARecParams: TRecParams);
    procedure SetaCamposCurrency(ATabela: TObject; ARecParams: TRecParams);
...
uses
  UAtributos, UDM, System.SysUtils, Vcl.Dialogs;
...
procedure TDAO.SetaCamposCurrency(ATabela: TObject; ARecParams: TRecParams);
begin
  with ARecParams do
  begin
    Prop.SetValue(ATabela, TFDQuery(Qry).Fields.FieldByName(Campo).AsCurrency);
  end;
end;

procedure TDAO.SetaCamposDate(ATabela: TObject; ARecParams: TRecParams);
begin
  with ARecParams do
  begin
    Prop.SetValue(ATabela, TFDQuery(Qry).Fields.FieldByName(Campo).AsDateTime);
  end;
end;

procedure TDAO.SetaCamposInteger(ATabela: TObject; ARecParams: TRecParams);
begin
  with ARecParams do
  begin
    Prop.SetValue(ATabela, TFDQuery(Qry).Fields.FieldByName(Campo).AsInteger);
  end;
end;

procedure TDAO.SetaCamposString(ATabela: TObject; ARecParams: TRecParams);
begin
  with ARecParams do
  begin
    Prop.SetValue(ATabela, TFDQuery(Qry).Fields.FieldByName(Campo).AsString);
  end;
...
begin
  ...
  case AProp.PropertyType.TypeKind of
    tkInt64, tkInteger:
    begin
      SetaCamposInteger(ATabela,Params);
    end;
    tkChar, tkString, tkUString:
    begin
      SetaCamposString(ATabela, Params);
    end;
    tkFloat:
      begin
        if CompareText(AProp.PropertyType.Name, 'TDateTime') = 0 then
          SetaCamposDate(ATabela,Params)
        else
          SetaCamposCurrency(ATabela,Params);
      end;
  else
